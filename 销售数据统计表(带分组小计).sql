CREATE PROCEDURE XN_FM."MTC_COST_95Report_SaleCostRevenlues_02"
(
	...
	TEMP = 				 
	 	SELECT T10."lastName"||T10."firstName" as "Creator",
			 N'应收发票' "DocType",
			   ...
			   IFNULL(T1."U_SlpName",T55."SlpName") "SlpName",
			   IFNULL(T1."U_BusiUnit",T3."U_RegSupName") "BusiUnit",
			   IFNULL(T1."U_SaleUnit",T3."U_SupMangName") "SaleUnit",
			   IFNULL(T81."Name",T31."Name") "SArea",
			   IFNULL(T82."Name",T32."Name") "SDept",
			   IFNULL(T83."Name",T33."Name") "SDimCd",
			   T3."AddID"||N'-'||T3."U_CustClass3" "SScale"--上级客户规模拼接客户分类三,徐浩3月20日更新
		...
		FROM OINV T0
			...
			LEFT JOIN OSLP T55 ON T3."SlpCode" = T55."SlpCode"
		...
		SELECT t10."lastName"||t10."firstName" as "Creator",
		      N'应收贷项凭证' "DocType",
			   ...
			   IFNULL(T82."Name",T32."Name") "SDept",
			   IFNULL(T83."Name",T33."Name") "SDimCd",
			   T3."AddID" "SScale"--上级客户规模,徐浩3月12日更新
			  ,T51."DfltIncom",T52."AcctName",T63."TransId"
			  ,CASE
			  WHEN T63."U_DocType"='1' THEN N'现'
			  WHEN T63."U_DocType"='2' THEN N'银'
